...
void LCD_Scan_Dir(u8 dir)
{
	...
	dirreg=0X36;
 	//regval|=0X08;//5310/5510/1963???BGR   
	LCD_WR_REG(lcddev.setxcmd); 
...
{ 	
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM4, ENABLE);  // 使能BANK1 
	...
	if(lcddev.id==0X9488)//如果是这几个IC,则设置WR时序为最快
	{
		...
		FSMC_Bank1E->BWTR[6]|=3<<0;		//地址建立时间(ADDSET)?3?HCLK =18ns  	 
		FSMC_Bank1E->BWTR[6]|=2<<8; 	//数据保存时间(DATAST)?6ns*3?HCLK=18ns
	}
 	printf(" LCD ID:%x\r\n",lcddev.id); //??LCD ID   
	if(lcddev.id==0X9488)	//9488???
	{	 
